...
def make_parser():
    ...
    parser.add_argument('-utr', '--utr', default=None, type=str, help="File containing sequences of 5'utr and terminator.")
...
def train_model(args):
    ...
    dataset = build_dataset(args.sequence_data, args.expression_data, args.name, encoding,
                            'training', bpps_file=args.base_pairing_data, algorithm=algorithm,
    ...
    if len(representative_feature_vector) != 0:
        dataset.train_model(algorithm, args.output_folder)
        fi_dir = os.path.join(args.output_folder, 'feature_importances')
        plots_dir = os.path.join(args.output_folder, 'plots')
        if not os.path.exists(plots_dir):
            os.mkdir(plots_dir)
        dataset.write_feature_importances(fi_dir)
        dataset.plot_feature_importances(args.output_folder)
